...
function App() {
  ...
  useEffect(() => {
    ...
    const handleDomReady = () => {
      ...
      // Override getDisplayMedia pour utiliser la modal Electron (LiveKit screen share)
      webview.executeJavaScript(`
        (function() {
          if (!window.__electronScreenPicker__) return;
          navigator.mediaDevices.getDisplayMedia = async function(_constraints) {
            const sourceId = await window.__electronScreenPicker__.pick();
            if (!sourceId) {
              throw new DOMException('Screen share annulé', 'NotAllowedError');
            }
          ...
          console.log('[Konex] Screen share prêt');
        })();
      `).catch(() => {});
    };
